...
import io.jenkins.plugins.ct.AbstractJacocoTestBase;
import io.jenkins.plugins.ct.model.CoverageGraphLayout;
...
public class CoverageObjectGraphTest extends AbstractJacocoTestBase {
	...
	public void simpleLineCoverage() throws IOException	{
		CoverageGraphLayout layout = new CoverageGraphLayout()
				/*.baseStroke(4f)*/
				.plot().type(CoverageType.STATEMENT).value(CoverageValue.MISSED).color(Color.RED)
				.plot().type(CoverageType.STATEMENT).value(CoverageValue.COVERED).color(Color.GREEN);
	...
	public void multipleAccessAndDifferentCoverageType() throws IOException {
		CoverageGraphLayout layout = new CoverageGraphLayout()
				...
				.plot().type(CoverageType.STATEMENT).value(CoverageValue.PERCENTAGE).color(Color.YELLOW);
