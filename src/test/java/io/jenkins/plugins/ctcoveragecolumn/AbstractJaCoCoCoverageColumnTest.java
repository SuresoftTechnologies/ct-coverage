...
import io.jenkins.plugins.ctcoveragecolumn.AbstractJaCoCoCoverageColumn;
import io.jenkins.plugins.ctcoveragecolumn.CoverageRange;
		...
		final Job<?, ?> mockJob = new MyJob("externaljob") {
			...
			public MyRun getLastSuccessfulBuild() {
				try {
					...
					newBuild.addAction(new CTBuildAction(null, null, null, StreamTaskListener.fromStdout(), null, null));
	...
	private final class ExternalJobExtensionWithBuildAction extends ExternalJobExtension {
		...
		public MyRun getLastSuccessfulBuild() {
			try {
				...
				run.addAction(new CTBuildAction(map, null, null, listener, null, null));
				return run;
			} catch (IOException e) {
				throw new IllegalStateException(e);
			}
